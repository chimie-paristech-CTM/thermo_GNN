...
class ThermoGNNAtomFeaturizer(VectorFeaturizer[Atom]):
    def __init__(self, max_atomic_num: int = 100):
        self.atomic_nums = {i: i-1 for i in range(1, max_atomic_num + 1)}
        self.degrees = {i: i for i in range(6)}
        self.chiral_tags = {i: i for i in range(4)}
        self.num_Hs = {i: i for i in range(5)}
...
def get_multi_hot_atom_featurizer(mode: str | AtomFeatureMode) -> MultiHotAtomFeaturizer:
    ...
    match AtomFeatureMode.get(mode):
        ...
        case AtomFeatureMode.THERMO_GNN:
            return ThermoGNNAtomFeaturizer()
        case _:
