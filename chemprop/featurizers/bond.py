...
class ThermoGNNBondFeaturizer(VectorFeaturizer[Bond]):
    def __init__(self):
        self.stereo_types = {i: i for i in range(6)}
        self.__size = 9
    ...
    def __call__(self, b: Bond | None) -> np.ndarray:
        ...
        bt = b.GetBondType()
        x[0] = 0
        x[1] = int(b.GetIsConjugated() if bt is not None else 0)
        x[2] = int(b.IsInRing() if bt is not None else 0)

        stereo = int(b.GetStereo())
        j = self.stereo_types.get(stereo, len(self.stereo_types))
        x[3 + j] = 1
