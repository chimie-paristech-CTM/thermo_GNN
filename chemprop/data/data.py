...
class MoleculeDatapoint:
    ...
    def __init__(self,
                 ...
                 smiles_label: List[Optional[str]] = None,
                 targets_label: List[Optional[str]] = None,
                 input_features_type: str = None,
        ...
        :param smiles_label: A list of the SMILES labels for the dataset_name, it will be used to calculate the joint loss
        :param targets_label: A list of the target labels for the dataset_name(the train dataset second columns), it will be used to calculate the joint loss
...
class MoleculeDataset(Dataset):
    ...
    def __init__(self, data: List[MoleculeDatapoint]):
        ...
        self._random = Random()
        # get the train args from data.utils file and it will used to control which feature to used in
        # make sure data list is not empty，and at least have one element
        if not data:
            raise ValueError("data list can't be empty")

        # Check if the first element in the data has the input_features.type attribute
        first_element = data[0]
        if not hasattr(first_element, 'input_features_type'):
            raise AttributeError("the first element in list lack the input_features_type attribute")

        # Assign the input_features.type attribute value to self_ Args
        self.input_features_type = first_element.input_features_type
    ...
    def smiles_label(self) -> List[List[Optional[str]]]:
        """
        Returns the targets associated with each molecule.

        :return: A list of lists of str (or None) containing the targets.
        """
        return [d.smiles_label for d in self._data]
    ...
    def batch_graph(self) -> List[BatchMolGraph]:
        ...
        if self._batch_graph is None:
            ...
            for d in self._data:
                ...
                for s, m in zip(d.smiles, d.mol):
                    if s in SMILES_TO_GRAPH:
                        ...
                        mol_graph = MolGraph(m, self.input_features_type, d.atom_features, d.bond_features,
            ...
            # passing args to control the input feature type
            self._batch_graph = [BatchMolGraph([g[i] for g in mol_graphs], self.input_features_type) for i in
                                 range(len(mol_graphs[0]))]
    ...
    def targets_label(self) -> List[List[Optional[float]]]:
        """
        Returns the targets label (the third column and used to calute the differet accuracy level loss) associated with each molecule.

        :return: A list of lists of str (or None) containing the targets.
        """
        return [d.targets_label for d in self._data]
