...
class BatchMolGraph:
    ...
    f_mol: Tensor | None = field(init=False, default=None)
    f_mol_atom: Tensor | None = field(init=False, default=None)
    ...
    def __post_init__(self, mgs: Sequence[MolGraph]):
        ...
        for i, mg in enumerate(mgs):
            ...
            if hasattr(mg, 'f_mol') and mg.f_mol is not None:
                f_mols.append(mg.f_mol)
                if hasattr(mg, 'f_mol_atom') and mg.f_mol_atom is not None:
                    f_mol_atoms.extend(mg.f_mol_atom)
                else:
                    f_mol_atoms.extend([mg.f_mol] * len(mg.V))
        ...
        if f_mols:
            self.f_mol = torch.from_numpy(np.array(f_mols)).float()
            self.f_mol_atom = torch.from_numpy(np.array(f_mol_atoms)).float()
        else:
            self.f_mol = None
            self.f_mol_atom = None
    ...
    def to(self, device: str | torch.device):
        ...
        if self.f_mol is not None:
            self.f_mol = self.f_mol.to(device)
            self.f_mol_atom = self.f_mol_atom.to(device)
