...
class MPNEncoder(nn.Module):
    ...
    def __init__(self, args: TrainArgs, atom_fdim: int, bond_fdim: int, mol_fdim: int, hidden_size: int = None,
    ...
    def forward(self,
        ...
        if atom_descriptors_batch is not None:
            atom_descriptors_batch = [np.zeros([1, atom_descriptors_batch[0].shape[
        ...
        if atom_descriptors_batch is not None:
            ...
            atom_hiddens = torch.cat([atom_hiddens, atom_descriptors_batch],
        ...
        if self.is_atom_bond_targets and bond_descriptors_batch is not None:
            ...
            bond_hiddens = torch.cat([bond_hiddens, bond_descriptors_batch],
                                     dim=1)  # num_bonds x (hidden + descriptor size)
        ...
        if self.output_fingerprint == "atom":
            for i, (a_start, a_size) in enumerate(a_scope):
                if a_size == 0:
                    atom_vecs.append(self.cached_zero_vector)
                else:
            ...
            f_mol_tensor = self.fdim_h(f_mol) # (num_molecules, hidden_size)
            mol_vecs,_ = self.wave_readout(mol_vecs, f_mol_tensor)
...
class MPN(nn.Module):
    ...
    def __init__(self,
        ...
        :param atom_fdim: Atom feature vector dimension.
        :param bond_fdim: Bond feature vector dimension.
        :param mol_fdim: Molecular feature vector dimension.
        """
        ...
        if not self.reaction_solvent:
            ...
            self.encoder = MPNEncoder(args, self.atom_fdim, self.bond_fdim, self.mol_fdim)
    ...
    def forward(self,
        ...
        if type(batch[0]) != BatchMolGraph:
            ...
            if self.atom_descriptors == 'feature':
                ...
                batch = [mol2graph(b,args=self.args) for b in batch]
