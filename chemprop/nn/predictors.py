...
class EvidentialFFN(RegressionFFN):
    ...
    def forward(self, Z: Tensor) -> Tensor:
        ...
        # Add minimum value constraints to prevent NaN (matching chemprop v1.x behavior)
        min_val = 1e-6
        v = F.softplus(v) + min_val
        alpha = F.softplus(alpha) + min_val + 1
        beta = F.softplus(beta) + min_val
