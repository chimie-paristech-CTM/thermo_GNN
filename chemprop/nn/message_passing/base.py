...
class _MessagePassingBase(MessagePassing, HyperparametersMixin):
    ...
    * :class:`BondMessagePassing`
    ...
    def __init__(
        ...
        use_mlp_trigonometric: bool = False,
    ):
        super().__init__()
        ...
        self.W_i, self.W_h, self.W_o, self.W_d = self.setup(d_v, d_e, d_h, d_mol, d_vd, bias)
        self.depth = depth
        ...
        # Initialize MlpTrigonometric module if requested
        self.use_mlp_trigonometric = use_mlp_trigonometric
        if self.use_mlp_trigonometric:
            from chemprop.nn.message_passing.mlp_trigonometric import MlpTrigonometric
            self.mlp_trig = MlpTrigonometric(d_h)
        else:
            self.mlp_trig = None
    ...
    def finalize(self, M: Tensor, V: Tensor, f_mol: Tensor | None, V_d: Tensor | None) -> Tensor:
        ...
        :math:`\mathbf{W}_o` and :math:`\mathbf{W}_d` are learned weight matrices, :math:`M` is the message matrix, :math:`V` is the original
        ...
        if f_mol is not None:
            H = self.W_o(torch.cat((f_mol, V, M), dim=1))
        else:
            H = self.W_o(torch.cat((V, M), dim=1))
        H = self.tau(H)
        H = self.dropout(H)
    ...
    def forward(self, bmg: BatchMolGraph, V_d: Tensor | None = None) -> Tensor:
        ...
        for _ in range(1, self.depth):
            ...
            if self.use_mlp_trigonometric and self.mlp_trig is not None:
                M = self.mlp_trig(M, H_original)
        ...
        f_mol = bmg.f_mol_atom if hasattr(bmg, 'f_mol_atom') else None

        return self.finalize(M, bmg.V, f_mol, V_d)
...
class BondMessagePassing(_BondMessagePassingMixin, _MessagePassingBase):
    ...
    def setup(
        ...
        W_o = nn.Linear(d_mol + d_v + d_h, d_h)
